...
* Submit your patches to `shell@fall2026-uml.kdlp.underground.software`
...
0. Flesh out the rootfs with utilities from `busybox` you can launch with your shell
    ...
    * Clone our fork of busybox from the repositories on the class website and cd into the folder
            ...
            git clone https://fall2026-uml.kdlp.underground.software/cgit/busybox
            cd busybox
    ...
    * Compile busybox. The existing config assumes you have the `sysroot` with libc from the previous assignment in your home directory `~`
...
0. Create your shell program that will act as `/bin/sh`
    ...
    * Implement the features outlined below

        * They are laid out in order increasing in sophisitication

        * Complete and thoroughly test each bullet point before moving on to the next

        * **Read them all before starting** to get the big picture so you can design each step with a mind for how what you are building will support what comes next

    * You can test the shell by running it inside the container without needing to set up the VM by relying on QEMU userspace emulation

        * If you prefix an invocation of a binary compiled for your RISC-V virtual machine with `qemu-riscv64-static` (e.g. `qemu-riscv64-static ./shell`) you can run it without having to boot the full VM

        * QEMU emulates the userspace code and translates the system calls to equivalent versions on the architecture of your host
        ...
        * You can pass `-g 1234` bewtween `qemu-riscv64-static` and the name of the binary to make the emulator wait for a debugger connection
            ...
            * Run `gdb` passing the path to the binary (so it can let you step through the code) and attach to the program being emulated with `target remote localhost:1234`

            * Consult the gdb documentation for details, but you can set breakpoints, single step, print variables, examine memory, etc
...
0. Try running your shell it in the VM
    ...
    * Re run the `qemu-system-riscv64` command and capture the output for your second commit

        *  Run the following commands within the shell once the kernel has booted
