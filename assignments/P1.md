...
* Patch 2 adds `username/P1/syscall.c`, a C program which invokes your new syscall, and `username/P1/Makefile`, which builds an executable named `syscall_C` via the default target
...
* Patch 3 adds `username/P1/syscall.s`, an assembly program which invokes your new syscall, and modifies `username/P1/Makefile` to build `syscall_asm` via the default target in addition to `syscall_C`
...
0. Start by adding a new system call to your copy of the linux kernel
	...
	* Otherwise, returns the amount of data that was copied
    ...
    0. Update the extra version tag in the main kernel Makefile so you can tell this kernel that has your system call apart from the other kernel you compiled
