...
You must adjust the indicated lines in the original file, fix your commit, and regenerate the patches.

Your patches also must apply cleanly to the `HEAD` commit on the `master` branch of the
[upstream submissions repository](https://fall2026-uml.kdlp.underground.software/cgit/ILKD_Submissions/).
...
0. If no errors appear: congratulations, your patchset applies cleanly!
    ...
    * If there are whitespace errors or corrupt patches, revise as needed by rebasing and amending your commits then regenerate the patches
