...
1. *CLARIFICATION*: When re-submitting your peer review, reply to the original cover and not your own email. When grading your peer review, we will only look at the latest reply to the latest cover letter.
...
1. Please don't hesitate to ask any questions in #questions. There are no stupid questions
...
1. As a reminder, busybox is a single binary containing many common Linux CLI utilities like `mkdir`, `ln`, and `cat`
    ...
    1. We can use the `mount` utility to add /proc to our system, and the kernel immediately provides what is needed.

    1. Likewise for the sysfs filesystem mounted at /sys

    1. We can add commands to our init script to mount these filesystems automatically

1. Today, we add a C compiler and C library to our system
...
1. Now, we to compile and install these various components into a usable filesystem.

    1. We create a directory tree to package as a cpio archive that we can use as the kernel's initial filesystem (initramfs)

    1. Right now, any executable binaries must be statically linked because our system doesn't yet have a dynamic linker
    ...
    1. `glibc` has a reputation for being difficult to build from source, but we have relatively few obstacles to deal with
...
1. We compile and run a "Hello world" C program to demonstrate that our system works
    ...
    1. With this fixes, `tcc` works as expected and we build and run "hello world" successfully
...
Here's a rundown of the commands and scripts we use in this demo
